...
export function useBgOverlayListener(): number {
  const [val, setVal] = useState(0); // 25% mặc định
  useEffect(() => {
    const handler = (e: Event) => setVal((e as CustomEvent<number>).detail);
...
export default function LayoutWidthControl() {
  ...
  const [cardsX, setCardsX] = useState(96);
  ...
  const [mediaList, setMediaListState] =
    useState<MediaItem[]>(defaultMediaList);
  const [showLayout, setShowLayout] = useState(true);
  const [showCard, setShowCard] = useState(false);
  const [showStyle, setShowStyle] = useState(false);
  const [showTypography, setShowTypography] = useState(false);
  const [showMediaSettings, setShowMediaSettings] = useState(false);
  const [servicesTemplate, setServicesTemplateState] =
    useState<ServicesTemplate>("v2");
  const [projectsTemplate, setProjectsTemplateState] =
    useState<ProjectsTemplate>("v2");
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(
      new CustomEvent(CARD_SIZE_EVENT, { detail: cardSize / 100 }),
    );
  ...
  const setHeroMode = (m: HeroMode) => {
    setHeroModeState(m);
    window.dispatchEvent(new CustomEvent(HERO_MODE_EVENT, { detail: m }));
  };
  const setQuoteStyle = (s: QuoteStyle) => {
    setQuoteStyleState(s);
    window.dispatchEvent(new CustomEvent(QUOTE_STYLE_EVENT, { detail: s }));
  };
  const setCardMode = (m: CardMode) => {
    setCardModeState(m);
    window.dispatchEvent(new CustomEvent(CARD_MODE_EVENT, { detail: m }));
  };
  const setLayoutMode = (m: LayoutMode) => {
    setLayoutModeState(m);
    window.dispatchEvent(new CustomEvent(LAYOUT_MODE_EVENT, { detail: m }));
  };
  const setServicesTemplate = (t: ServicesTemplate) => {
    setServicesTemplateState(t);
    window.dispatchEvent(
      new CustomEvent(SERVICES_TEMPLATE_EVENT, { detail: t }),
    );
  };
  const setProjectsTemplate = (t: ProjectsTemplate) => {
    setProjectsTemplateState(t);
    window.dispatchEvent(
      new CustomEvent(PROJECTS_TEMPLATE_EVENT, { detail: t }),
    );
  };
  ...
  const handleBgOverlay = (v: number) => {
    setBgOverlay(v);
    window.dispatchEvent(new CustomEvent(BG_OVERLAY_EVENT, { detail: v }));
  };
  const handleCardDim = (v: number) => {
    setCardDim(v);
    window.dispatchEvent(new CustomEvent(CARD_DIM_EVENT, { detail: v }));
  };
  const handleCardVignette = (v: boolean) => {
    setCardVignette(v);
    window.dispatchEvent(new CustomEvent(CARD_VIGNETTE_EVENT, { detail: v }));
  };
  ...
  const handleMediaUpload = (
    ...
    window.dispatchEvent(
      new CustomEvent(MEDIA_LIST_EVENT, { detail: newList }),
    );
  };
        ...
        <div className="bg-black/93 border border-white/12 backdrop-blur-md rounded-xl px-5 py-5 flex flex-col gap-4 shadow-2xl w-[300px] max-h-[85vh] overflow-y-auto">
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              ...
              <svg
                className={`w-3 h-3 transition-transform ${showLayout ? "rotate-180" : ""}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              ...
              <svg
                className={`w-3 h-3 transition-transform ${showCard ? "rotate-180" : ""}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            {showCard && (
              <div className="flex flex-col gap-3 animate-in fade-in slide-in-from-top-1 duration-200">
                ...
                <div className="flex items-center justify-between py-1">
                  ...
                  >
                    <div
                      className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-all ${cardVignette ? "left-5" : "left-0.5"}`}
                    />
                  </button>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-col gap-1">
                    {[
                      ...
                      <button
                        key={opt.id}
                        onClick={() => setCardMode(opt.id as CardMode)}
                        className={`w-full py-1.5 px-3 text-[10px] font-bold uppercase text-left rounded-lg border transition-all ${
                          cardModeState === opt.id
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              ...
              <svg
                className={`w-3 h-3 transition-transform ${showStyle ? "rotate-180" : ""}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                ...
                <div className="flex gap-2">
                  {[
                    ...
                    <button
                      key={opt.id}
                      onClick={() =>
                        setServicesTemplate(opt.id as ServicesTemplate)
                      }
                      className={`flex-1 py-1.5 text-[10px] font-bold uppercase rounded-lg border transition-all ${
                        servicesTemplate === opt.id
                          ? "bg-amber-400 border-amber-400 text-black"
                          : "border-white/15 text-white/50 hover:text-white hover:border-white/30"
                      }`}
                    >
                  ...
                  <div className="flex gap-2">
                    {[
                      ...
                      <button
                        key={opt.id}
                        onClick={() =>
                          setProjectsTemplate(opt.id as ProjectsTemplate)
                        }
                        className={`flex-1 py-1.5 text-[10px] font-bold uppercase rounded-lg border transition-all ${
                          projectsTemplate === opt.id
                            ? "bg-amber-400 border-amber-400 text-black"
                            : "border-white/15 text-white/50 hover:text-white hover:border-white/30"
                        }`}
                      >
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              ...
              <svg
                className={`w-3 h-3 transition-transform ${showTypography ? "rotate-180" : ""}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-top-1 duration-200">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <input
                    ...
                    className="w-full h-6 bg-white/5 border border-white/10 rounded cursor-pointer"
                  />
                </div>
              </div>
            )}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            >
              ...
              <svg
                className={`w-3 h-3 transition-transform ${showMediaSettings ? "rotate-180" : ""}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            {showMediaSettings && (
              <div className="flex flex-col gap-3 mt-1 animate-in fade-in slide-in-from-top-1 duration-200">
                {mediaList.map((media, idx) => (
                  <div
                    key={media.id}
                    className="flex flex-col gap-1.5 border-l-2 border-amber-500/30 pl-2"
                  >
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-[9px] text-white/70 bg-white/5 hover:bg-white/10 px-2 py-1 rounded cursor-pointer transition-colors border border-white/10 flex items-center justify-between">
                          ...
                          <input
                            ...
                            className="hidden"
                            onChange={(e) =>
                              handleMediaUpload(e, idx, "thumbnail")
                            }
                          />
                        </label>
                        <input
                          type="text"
                          placeholder="URL"
                          className="w-full bg-black/50 border border-white/10 text-white text-[9px] px-2 py-1 rounded outline-none focus:border-amber-500/50"
                          value={
                            media.thumbnail.startsWith("blob:")
                              ? ""
                              : media.thumbnail
                          }
                          onChange={(e) =>
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-[9px] text-white/70 bg-white/5 hover:bg-white/10 px-2 py-1 rounded cursor-pointer transition-colors border border-white/10 flex items-center justify-between">
                          ...
                          <input
                            ...
                            accept="video/*,image/*"
                            className="hidden"
                            onChange={(e) => handleMediaUpload(e, idx, "bg")}
                          />
                        </label>
                        <input
                          type="text"
                          ...
                          className="w-full bg-black/50 border border-white/10 text-white text-[9px] px-2 py-1 rounded outline-none focus:border-amber-500/50"
                          value={
                            media.path.startsWith("blob:") ? "" : media.path
                          }
                          onChange={(e) =>
                            handleMediaUrlChange(e.target.value, idx, "bg")
                          }
                        />
                      </div>
                    </div>
